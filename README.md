...
CVec是一个C语言实现的线性列表容器，类似于C++的 vector。它能在运行时改变容器大小，因此比数组更加灵活；它支持编译期的类型检查，因此比未模板化的数据结构更加安全。此外，它还支持可开关的边界检查功能。

# 使用速览

CVec只包含一个文件：vec.h，使用时`#include`它即可。CVec的基本原理是用宏生成结构体和函数的声明和定义。因此，您在使用CVec前要先调用宏来生成它们。

## 生成代码

首先，用`DECL_VEC(vtype, type, prefix, rqc, rqd)`宏生成结构体和函数原型声明。

`vtype`代表列表的结构体名称及相关函数名的开头部分，您可以取一个自己喜欢的名字（合法的 C 标识符）。

`type`代表列表中存储的数据类型，比如`int`、`float`或者`char*`。建议不要在这里加`const`限定符，否则编译器会报告warning。

`prefix`是所有生成函数的前缀。比如填写`static`将使所有生成出来的函数带上`static`前缀，从而使得它们具有内部链接。如果没有这方面的需求，可以留空此参数或用`EMPTY_PREFIX`占位。

`rqc`的值只能是`0`或`1`。它表示列表中的元素是需要一个自定义拷贝函数来完成拷贝，还是只需`=`运算符即可完成拷贝。`0`表示需要自定义拷贝函数，`1`表示不需要。拷贝会在以下情况执行：
...
`rqd`的值只能是`0`或`1`。它表示列表中的元素在被删除时是否需要自定义的析构函数来处理。`0`表示需要，`1`表示不需要。如果您将`rqc`的值设置为`1`，`rqd`的值一般也应该是`1`。

例如，这样可以生成存放`char*`的列表的结构体定义和API函数声明：
```
...
生成了`StrLs`相关代码，现在可以用生成出的`StrLs_init`函数创建一个列表。但在此之前，我们需要编写拷贝和析构字符串的函数，因为我们之前将`rqc`和`rqd`都指定为`1`了。
...
析构函数形参的类型也是传给宏的`type`参数，返回值是`void`。
...
```
有了它们，我们在`main`函数内部调用`StrLs_init`创建一个列表。
...
`StrLs_push_back`向表的尾部添加元素。第一个参数是表的指针，第二个参数是要添加的对象，类型与`type`相同（`char*`）。p.s. 只要在`copy_str`函数里不修改传入的指针指向的内存，就可以放心强转。
...
CVec可以通过以下宏进行配置。可以在#include "vec.h"前定义以下宏来进行配置。
...
列表存放在连续地址中，因此对该指针调用`++`运算符即可让它指向下一个元素（如果存在的话）,其他运算符同理。
